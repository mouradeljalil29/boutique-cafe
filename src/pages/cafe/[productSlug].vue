<template>
  ...
  <div v-else class="container">
    <p style="margin:60px 0">Produit introuvable. <NuxtLink to="/catalogue">Retour au catalogue</NuxtLink></p>
  </div>
</template>
...
// SEO réactif : les getter functions () => ... se mettent à jour automatiquement
// quand product.value change (navigation entre produits côté client)
setMeta({
  title: () => product.value?.metaTitle,
  description: () => product.value?.metaDescription,
  canonical: () => product.value ? `/cafe/${product.value.slug}` : null,
...
setJsonLD(() => {
  ...
  return [productSchema(product.value), bc]
})
</script>
