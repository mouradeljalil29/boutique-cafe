<template>
  ...
  <div v-else class="container">
    <p style="margin:40px 0">Catégorie introuvable. <router-link to="/catalogue">Retour au catalogue</router-link></p>
  </div>
</template>
...
import { categories, getProductsByCategory, getProductsBySubcategory, getCategoryBySlug } from '@/data/products.js'
...
const category = computed(() => getCategoryBySlug(route.params.categorySlug))
const categoryProducts = computed(() => category.value ? getProductsByCategory(category.value.id) : [])
...
onMounted(updateSEO)
watch(() => route.params.categorySlug, updateSEO)
</script>
